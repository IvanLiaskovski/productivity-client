...
const TaskUnDo = ({ taskId, className }) => {
  ...
  const isDayOrYear = useCheckTasksURL(["day", "year"]);

  const unDoStyles = twMerge(
    `text-bgBottom duration-400 absolute right-0 top-1/2 -translate-y-1/2 translate-x-full text-bgBottom 
    transition-transform hover:scale-110 hover:opacity-90 group-hover:-translate-x-1/2 z-10`,
    className,
  );

  const iconFontSize = isDayOrYear && "text-2xl";
